...
def parse_url(url):
    ...
    model = load_model(os.path.join(path,'models','JORN-14.tf'))
    ...
    classes=('CALPER2', 'CHLSER', 'CHLTOM', 'CICPUN', 'CYMPUN2', 'DISROB', 'EURGRO', 'HAROBL', 'HARPEN', 'HELFER', 'JUNLEM', 'PASOBS', 'SELPLA', 'TETPAL')
    ...
    dwc_det = {
            ...
            "identificationReferences" : "ID used the JORN-14 carabid model",
            "identificationRemarks" : f'Model probability is {str(round(y_prob[0][y_prob.argmax()],5))}'
        }
    ...
    #write output to log file
    log_file = os.path.join(path,'logs','JORN-14.log')
